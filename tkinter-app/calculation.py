...
def process_inputs(inputs):
    ...
    numeric_columns = ["precipitation", "wind"]
    ...
    Q1 = df3["precipitation"].quantile(0.25)
    Q3 = df3["precipitation"].quantile(0.75)
    IQR = Q3 - Q1
    df3 = df3[~((df3["precipitation"] < (Q1 - 1.5 * IQR)) | (df3["precipitation"] > (Q3 + 1.5 * IQR)))]
    ...
    # Convert "date" column to datetime format and then to int64
    df_new["date"] = pd.to_datetime(df_new["date"]).apply(lambda x: int(x.timestamp()))
